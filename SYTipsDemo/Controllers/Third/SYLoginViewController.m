...
- (void)loginBtnClicked:(UIButton *)sender{
    ...
    if (_pwdTextField.text.length >= 6) {
        ...
        if (![_pwdTextField.text isEqualToString:@"Sauchye"]) {
            ...
            [self shakeView];
            [SYHUDView showToView:self.view.window success:NO text:@"Login Failed" hide:2.0];
        }else{
            [self hideStatusLoading];
            _pwdTextField.layer.borderColor = [UIColor clearColor].CGColor;
            _userTextField.layer.borderColor = [UIColor clearColor].CGColor;
            [SYHUDView showToView:self.view.window success:YES text:@"Login Success" hide:2.0];
